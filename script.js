...
function initializeApp() {
    ...
    setTimeout(() => {
        ...
        console.log('✅ 페이지 로드 시 자동전송 설정 로드 완료');
    }, 200);
            ...
            ">
                <div style="margin-bottom: 20px;">
                    ...
                    <p style="margin: 0; color: #888; text-align: center; font-size: 14px;">
                        각 계정마다 다른 메시지를 선택할 수 있습니다. 계정을 클릭하여 메시지를 선택하세요
                    </p>
                </div>
                        ...
                        ">
                            <div style="display: flex; align-items: center; gap: 12px;">
                                ...
                                <div style="flex: 1;">
                                    ...
                                    <div class="account-message-status" style="color: #888; font-size: 11px; margin-top: 4px;">
                                        메시지 미선택
                                    </div>
                                </div>
...
function applyChannelMessageToSelectedAccount() {
    if (!window.selectedChannelMessage) return;
    
    // 현재 선택된 계정 ID 가져오기
    const selectedAccountId = localStorage.getItem('lastSelectedAccount');
    console.log('📢 선택된 채널 메시지를 계정에 적용:', selectedAccountId);
    
    if (!selectedAccountId) {
        console.error('❌ 선택된 계정 ID가 없습니다');
        return;
    }
...
function loadAutoSendSettings() {
    try {
        ...
        // DOM 요소가 없으면 로드하지 않음
        if (!groupInterval || !repeatInterval || !maxRepeats || !messageThreshold || !enableMessageCheck) {
            console.log('⚠️ 자동전송 설정 DOM 요소가 없음 - 로드 건너뜀');
            return;
        }
        ...
        if (settings) {
            ...
            if (settings.rotationPools && settings.rotationPools.enabled !== undefined) {
                window.rotationPoolsEnabled = settings.rotationPools.enabled;
                const enablePoolsCheckbox = document.getElementById('enableRotationPools');
                if (enablePoolsCheckbox) {
                    enablePoolsCheckbox.checked = settings.rotationPools.enabled;
                }
                console.log('✅ 풀 시스템 설정 복원:', settings.rotationPools.enabled);
            } else {
                // 저장된 풀 시스템 설정이 없으면 비활성화로 초기화
                window.rotationPoolsEnabled = false;
                const enablePoolsCheckbox = document.getElementById('enableRotationPools');
                if (enablePoolsCheckbox) {
                    enablePoolsCheckbox.checked = false;
                }
                console.log('✅ 풀 시스템 설정 없음 - 비활성화로 초기화');
            }
            ...
            // 기본값 설정
            if (groupInterval) {
                groupInterval.value = 30;
                console.log('✅ 그룹간 전송간격 기본값 설정: 30초');
            }
            if (repeatInterval) {
                repeatInterval.value = 30;
                console.log('✅ 반복 간격 기본값 설정: 30초');
            }
            if (maxRepeats) {
                maxRepeats.value = 10;
                console.log('✅ 최대 반복 기본값 설정: 10회');
            }
            if (messageThreshold) {
                messageThreshold.value = 5;
                console.log('✅ 메시지 임계값 기본값 설정: 5개');
            }
            if (enableMessageCheck) {
                enableMessageCheck.checked = true;
                console.log('✅ 메시지 확인 기본값 설정: true');
            }
    ...
    if (enablePoolsCheckbox) {
        // 초기 상태는 항상 비활성화로 설정
        if (window.rotationPoolsEnabled === undefined) {
            window.rotationPoolsEnabled = false;
        }
...
function saveAutoSendSettings() {
    ...
    // 풀 시스템이 활성화되어 있으면 반복전송 간격만 무의미한 값으로 설정 (나머지는 유지)
    if (window.rotationPoolsEnabled) {
        console.log('✅ 풀 시스템 활성화됨 - 반복전송 간격만 무의미한 값으로 설정');
        ...
        // maxRepeats는 여전히 필요하므로 유지
        // settings.maxRepeats = parseInt(maxRepeats); // 유지
...
window.rotationPoolsEnabled = false; // 풀 시스템 활성화 여부 (기본값: 비활성화)
    ...
    for (const checkbox of checkedBoxes) {
        ...
        for (const account of accounts) {
            try {
                ...
                console.log(`🔍 계정 ${account.first_name} - 마지막 계정 여부: ${isLastAccount}`);
                
                if (!isLastAccount) {
                    const pool = window.rotationPools[account.poolId];
                    console.log(`🔍 풀 정보:`, pool);
                    ...
                    if (pool && pool.rotationInterval) {
                        ...
                        console.log(`⚠️ 풀별 간격 설정이 없음 - poolId: ${account.poolId}, rotationInterval: ${pool?.rotationInterval}`);
                    }
                }
            } catch (error) {
...
async function loadChannelMessagesForMultiAccount(channelId) {
    ...
    try {
        ...
        if (result.messages && result.messages.length > 0) {
            ...
            // 미디어 타입이 있는 메시지 찾기
            const mediaMessage = result.messages.find(msg => msg.media_type);
            if (mediaMessage) {
                console.log('📥 미디어 메시지 상세:', {
                    id: mediaMessage.id,
                    media_type: mediaMessage.media_type,
                    media_info: mediaMessage.media_info,
                    mime_type: mediaMessage.media_info?.mime_type
                });
            }
        }
...
function displayChannelMessagesForMultiAccount(messages, channelTitle) {
    ...
    messages.forEach((message, index) => {
        ...
        const messagePreview = message.text && message.text.length > 100 ? 
            parseTelegramEntities(message.text.substring(0, 100), message.entities || []) + '...' : 
            parseTelegramEntities(message.text || '', message.entities || []);
