...
class FirebaseService {
    ...
    // 자동전송 설정 저장
    async saveAutoSendSettings(userId, settings) {
        try {
            console.log(`Firebase: 자동전송 설정 저장 - ${userId}`);
            ...
            const autoSendRef = ref(this.database, `auto_send_settings/${userId}`);
            await set(autoSendRef, autoSendData);
            
            console.log('Firebase: 자동전송 설정 저장 성공');
            return true;
        } catch (error) {
            console.error('Firebase 자동전송 설정 저장 실패:', error);
            return false;
        }
    }

    // 자동전송 설정 조회
    async getAutoSendSettings(userId) {
        try {
            console.log(`Firebase: 자동전송 설정 조회 - ${userId}`);
            
            const autoSendRef = ref(this.database, `auto_send_settings/${userId}`);
            const snapshot = await get(autoSendRef);
            
            if (snapshot.exists()) {
                const autoSendData = snapshot.val();
                console.log('Firebase: 자동전송 설정 조회 성공');
                return autoSendData;
            }
            
            console.log(`Firebase: 자동전송 설정 없음 - ${userId}`);
            return null;
        } catch (error) {
            console.error('Firebase 자동전송 설정 조회 실패:', error);
            return null;
        }
    }

    // 자동전송 상태 저장
    async saveAutoSendStatus(userId, status) {
        try {
            console.log(`Firebase: 자동전송 상태 저장 - ${userId}`);
            ...
            const statusRef = ref(this.database, `auto_send_status/${userId}`);
    ...
    // 자동전송 상태 조회
    async getAutoSendStatus(userId) {
        try {
            console.log(`Firebase: 자동전송 상태 조회 - ${userId}`);
            ...
            console.log(`Firebase: 자동전송 상태 없음 - ${userId}`);
            return null;
        } catch (error) {
    ...
    // 로테이션 풀 설정 저장
    async saveRotationPools(userId, poolsData) {
        try {
            console.log(`Firebase: 로테이션 풀 설정 저장 - ${userId}`);
            ...
            const poolsRef = ref(this.database, `rotation_pools/${userId}`);
            await set(poolsRef, poolsInfo);
            
            console.log('Firebase: 로테이션 풀 설정 저장 성공');
            return true;
        } catch (error) {
            console.error('Firebase 로테이션 풀 설정 저장 실패:', error);
            return false;
        }
    }

    // 로테이션 풀 설정 조회
    async getRotationPools(userId) {
        try {
            console.log(`Firebase: 로테이션 풀 설정 조회 - ${userId}`);
            ...
            if (snapshot.exists()) {
                ...
                console.log('Firebase: 로테이션 풀 설정 조회 성공');
                return poolsData;
            }
            
            console.log(`Firebase: 로테이션 풀 설정 없음 - ${userId}`);
            return null;
        } catch (error) {
            console.error('Firebase 로테이션 풀 설정 조회 실패:', error);
            return null;
        }
    }
